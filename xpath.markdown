...
*   Простые `child::` (сокращённо пишется просто `parent-item/child-item`,
    но можно писать `parent-item/child::child-item`) и `descendant::` (сокращённо пишется через `//` ---
    `parent-item//descendant-item`, но можно писать `parent-item/descendant::descendant-item`).
...
*   `descendant-or-self::` содержит текущую ноду и всех потомков в порядке отдаления "родства".
