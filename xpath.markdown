...
*   Простые `child::` (сокращённо пишется просто `parent-item/child-item`, но можно писать `parent-item/child::child-item`) и `descendant::` (сокращённо пишется через `//` --- `parent-item//descendant-item`, но можно писать `parent-item/descendant::descendant-item`).
...
*   `self::` содержит только контекстную ноду, сокращённо можно писать через `.`
*   `descendant-or-self::` содержит текущую ноду всех потомков в порядке отдаления "родства"
...
**TODO:** Хороших примеров  following и preceding.
